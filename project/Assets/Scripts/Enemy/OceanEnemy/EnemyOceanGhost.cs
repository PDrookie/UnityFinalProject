...
public class EnemyOceanGhost : OceanEnemy
{
    public float Speed;
    ...
    public void Update()
    {
        ...
        {
            PlayerCome = true;
            transform.position = Vector2.MoveTowards(this.transform.position, player.position, Speed * 3 *  Time.deltaTime);
        }
    ...
    void flipTheBody()
    {
        float GhostDirection = movePos.position.x - transform.position.x;
        //Debug.Log(flyeyeDirection);

        if (GhostDirection > 0 && movingLeft)
        {
            flip();
        }
        else if (GhostDirection < 0 && !movingLeft)
        {
            flip();
        }
    }
